...
def validate_claim_file(path: Path, domain_ids: set[str], relation_ids: set[str]) -> tuple[str | None, list[str]]:
    errors: list[str] = []
    where = str(path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for path in claim_paths:
        claim_id, claim_errors = validate_claim_file(path, domain_ids, relation_ids)
