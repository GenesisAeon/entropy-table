...
def validate_claim_file(
            ...
            if not isinstance(cases, list):
                ...
                for i, item in enumerate(cases):
                    if isinstance(item, str):
                        if not item.strip():
                            errors.append(f"{where}: evidence.cases[{i}] cannot be an empty string")
                    elif isinstance(item, dict):
                        if "id" not in item or not isinstance(item["id"], str) or not item["id"].strip():
                            errors.append(f"{where}: evidence.cases[{i}] missing or invalid 'id'")
                    else:
                        errors.append(f"{where}: evidence.cases[{i}] must be a string or a dictionary")
    ...
    if status in {"stable", "review"} and isinstance(evidence, dict) and isinstance(evidence.get("cases"), list):
        for item in evidence["cases"]:
            if isinstance(item, dict) and "compute_ref" in item:
                ...
                try:
                    ...
                    if hasattr(mod, "verify_claim"):
                        if not mod.verify_claim():
                            errors.append(f"{where}: compute_ref '{compute_ref}' returned False")
                    else:
                        errors.append(f"{where}: compute_ref '{compute_ref}' missing verify_claim() function")
                except Exception as e:
                    errors.append(f"{where}: compute_ref '{compute_ref}' execution failed: {e}")
